...
ASP.NET MVC 路由根据 URL 来定义，而在 ASP.NET Web API中，路由基于 HTTP 方法。GET 请求路由得到 Get 开头的控制器方法，POST 请求路由得到 Post 开头的控制器方法。
...
这个路由非常类似于 ASP.NET MVC 路由定义。在 ASP.NET MVC中，路由由 MapRoute 方法定义。MapHttpRoute 方法不由路由URL链接，而是映射 HTTP 请求。

前面简单的命名约定将 HTTP 方法映射到动作方法。HTTP GET 请求映射到方法 GetBookChapter，HTTP POST 请求映射到方法 PostBookChapter。
...
如果动作方法的名称没有定义到 HTTP 方法的映射，也可以使用特性来定义。例如，一个方法名称是 BookChapters，就可以使用 HttpGet 特性把它映射到 HTTP GET 请求
...
与GET请求类似，HEAD、POST、PUT 和 DELETE 请求可以使用其他特性：HttpHead、HttpPost、HttpPut和HttpDelete。另一个选项是使用 AcceptVerb 特性。这个特性允许多个 HTTP 方法赋予一个动作方法。假设有一个与动作方法不同的 URL 链接，就可以使用 ActionName 特性
...
基于特性的路由的有点是，它在一个地方定义，不需要用控制器指定任何映射。但是它很难用于大量父子关系的控制器，要启动基于特性的路由，需要用注册路由调用 MapHttpAttributeRoutes 方法。在 Web API 2中，这个方法定义为扩展方法，位于程序集 System.Web.Http 的名称空间 System.Web.Http 中。
...
在基于特性的路由中，Route 特性可以应用于动作方法。如果应用了这个特性，就使用基于特性的路由。对于没有这种特性的动作方法，就使用基于约定的路由。有了 Route 特性，模板可以指定为在花括号中包含方法的参数。下面的路由应用于 URL http://service/books/2，其中 2 传递为 bookId参数
...
使用 Route 特性的模板，也可以传递多个参数。对于 URL http://server/books/1/chapters/5，返回图书中 id 为1的第5章
